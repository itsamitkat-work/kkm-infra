-- Fix custom_access_token_hook returning SQL NULL when any claim value is NULL.
--
-- `to_jsonb(NULL)` returns SQL NULL (not JSON null). `jsonb_set` returns NULL
-- when new_value is SQL NULL, which poisons `claims` and makes the hook return
-- NULL. GoTrue rejects with: output claims do not conform to the expected schema.
--
-- Common case: tenant_members exists but no role yet → active_role_slug NULL.
