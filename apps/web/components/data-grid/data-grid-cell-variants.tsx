...
import { useItemsSearch } from '@/hooks/items/use-items-search';
...
export function ItemCodeAutocompleteCell<TData>({
  cell,
  tableMeta,
  rowIndex,
  columnId,
  ...
  const cellOpts = cell.column.columnDef.meta?.cell;
  const fillColumns =
    cellOpts?.variant === 'item-autocomplete'
      ? (cellOpts.fillColumns ?? {})
      : {};
  const { items, isLoading } = useItemsSearch(searchQuery);
  ...
  const onChange = React.useCallback(
    (selectedCode: string, item: MasterItem) => {
      if (readOnly) return;
      const updates: Array<{
        rowIndex: number;
        columnId: string;
        value: unknown;
      }> = [{ rowIndex, columnId, value: item.code }];
      for (const [colId, itemKey] of Object.entries(fillColumns)) {
        const val = (item as unknown as Record<string, unknown>)[itemKey];
        if (val !== undefined)
          updates.push({ rowIndex, columnId: colId, value: val });
      }
      tableMeta?.onDataUpdate?.(updates);
      tableMeta?.onCellEditingStop?.();
    },
    [tableMeta, rowIndex, columnId, readOnly, fillColumns]
  );
    ...
    >
      {isEditing && !readOnly ? (
        <Combobox<MasterItem>
          ...
          loading={isLoading}
          getOptionId={(item) => item.code}
          getOptionLabel={(item) => `${item.code} - ${item.name}`}
          placeholder='Search items...'
          searchPlaceholder='Search by code or name...'
          className='h-full min-h-0 w-full border-none px-2 shadow-none focus-visible:ring-0'
        />
      ) : (
...
export function ItemOrMaterialSelectCell<TData>({
  cell,
  tableMeta,
  rowIndex,
  columnId,
  ...
  const onSelectItem = React.useCallback(() => {
    (
      tableMeta as { onRequestItemSelect?: (rowIndex: number) => void }
    )?.onRequestItemSelect?.(rowIndex);
    setOpen(false);
  }, [tableMeta, rowIndex]);

  const onSelectMaterial = React.useCallback(() => {
    (
      tableMeta as { onRequestMaterialSelect?: (rowIndex: number) => void }
    )?.onRequestMaterialSelect?.(rowIndex);
    setOpen(false);
  }, [tableMeta, rowIndex]);
        ...
        <Popover open={open} onOpenChange={onOpenChange}>
          <PopoverTrigger asChild>
            <button
              type='button'
              data-slot='grid-cell-content'
              className='h-full min-h-0 w-full truncate px-2 text-left outline-none focus-visible:ring-2 focus-visible:ring-ring'
            >
              {displayValue || ''}
            </button>
          </PopoverTrigger>
