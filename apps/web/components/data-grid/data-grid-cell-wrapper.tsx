...
export function DataGridCellWrapper<TData>({
  ...
  return (
    <div
      ...
      ref={composedRef}
      className={cn(
        'bg-muted/50 text-muted-foreground size-full text-start text-sm outline-none has-data-[slot=checkbox]:pt-2.5',
        rowHeight === 'compact' ? 'px-1.5 py-0.5' : 'px-2 py-1.5',
        {
          ...
          'bg-amber-50/30 dark:bg-amber-900/10 text-foreground':
            !readOnly && !isSelected && !isEditing && !isActiveSearchMatch,
          'opacity-70': readOnly && !isEditing,
          'cursor-default': !isEditing,
          '**:data-[slot=grid-cell-content]:line-clamp-1':
            !isEditing && (rowHeight === 'compact' || rowHeight === 'short'),
