...
/** Fixed width so leaf rows align with folder rows (icon vs empty slot). */
...
function TreeItemLabel<T = any>({
  ...
  return (
    <Comp
      ...
      className={cn(
        "in-focus-visible:ring-ring/50 bg-background hover:bg-accent in-data-[selected=true]:bg-accent in-data-[selected=true]:text-accent-foreground in-data-[drag-target=true]:bg-accent flex items-center gap-1 transition-colors in-focus-visible:ring-[3px] in-data-[search-match=true]:bg-blue-50! [&_svg]:pointer-events-none [&_svg]:shrink-0",
        "rounded-sm",
        "py-1.5",
        "px-2",
        "text-sm",
    ...
    >
      <Fragment>
        <span
          className={TREE_TOGGLE_CELL_CLASS}
          aria-hidden={!item.isFolder()}
        >
          {toggleIconType === "plus-minus" ? (
            item.isFolder() ? (
              item.isExpanded() ? (
