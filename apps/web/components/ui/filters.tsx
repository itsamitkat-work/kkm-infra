  ...
  "text-muted-foreground shrink-0 flex items-center",
  {
    variants: {
      variant: {
        ...
        readonly: "border border-border border-x-0 cursor-default",
      },
      size: {
        lg: "h-10 px-4 text-sm",
        md: "h-9 px-3 text-sm",
...
export interface FilterFieldConfig<T = unknown> {
  ...
  onLabel?: string;
  offLabel?: string;
  // Required/non-removable filter
  required?: boolean;
  // Disabled state (e.g. dependent on another filter)
  disabled?: boolean;
}
...
function FilterOperatorDropdown<T = unknown>({
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger
        className={cn(filterOperatorVariants({
          variant: context.variant,
          size: context.size,
        }), shouldDisable && 'hover:bg-transparent cursor-default')}
...
function SelectOptionsPopover<T = unknown>({
  ...
  return (
    ...
    >
        ...
        <div className="flex gap-1.5 items-center">
          {field.customValueRenderer ? (
            ...
            <>
              ...
              <span className={cn('text-xs', !selectedOptions.length && 'text-muted-foreground')}>
      ...
      <PopoverContent
        align="start"
        className={cn("w-[calc(100vw-32px)] max-w-md lg:min-w-[400px] lg:w-auto p-0", field.popoverContentClassName)}
      >
        <Command>
          {field.searchable !== false && (
            <CommandInput
...
function FilterValueSelector<T = unknown>({
  ...
  // Disabled state: show placeholder only, no interaction
  if (field.disabled) {
    return (
      <div
        className={cn(
    ...
    return (
      ...
      >
        <FilterInput
          ...
          className={cn("w-30", field.className)}
          field={field}
        />
        <div
          data-slot="filters-between"
...
export const FiltersContent = <T = unknown,>({
  ...
  const removeFilter = useCallback(
    (filterId: string) => {
      ...
      const field = fieldsMap[filterToRemove?.field || ''];
      // Don't remove required filters
      if (field?.required) {
        return;
      }
      ...
      {filters.map((filter) => {
        ...
        return (
          ...
          >
            ...
            {/* Remove Button - disabled for required filters */}
            <FilterRemoveButton
          ...
          return (
            ...
            >
              ...
              <FilterRemoveButton
                onClick={() => removeFilter(filter.id)}
                disabled={field.required}
                className={field.required ? 'cursor-not-allowed pointer-events-none opacity-50' : ''}
              />
            </div>
          );
        })}
