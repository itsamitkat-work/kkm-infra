...
const numberInputNoStepClasses =
...
function Input({
  ...
  type,
  variant,
  disableStepArrows = true,
  onKeyDown,
  ...props
}: React.ComponentProps<'input'> &
  VariantProps<typeof inputVariants> & {
    /** When true, hides number spinner and prevents ArrowUp/ArrowDown from changing value (for type="number"). */
    disableStepArrows?: boolean;
  }) {
  const handleKeyDown = React.useCallback(
    (e: React.KeyboardEvent<HTMLInputElement>) => {
      if (disableStepArrows && (e.key === 'ArrowUp' || e.key === 'ArrowDown')) {
        e.preventDefault();
      }
      onKeyDown?.(e);
    },
    [disableStepArrows, onKeyDown]
  );
  ...
  return (
    <input
      ...
      className={cn(
        ...
        disableStepArrows && numberInputNoStepClasses,
        className
      )}
      onKeyDown={disableStepArrows ? handleKeyDown : onKeyDown}
      {...props}
    />
  );
}
