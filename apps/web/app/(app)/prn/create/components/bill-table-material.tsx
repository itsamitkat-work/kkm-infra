...
import { ServiceItemPrnTable } from './service-item-prn-table';

function formatCurrency(value: number): string {
  return `₹ ${value.toLocaleString('en-IN', { minimumFractionDigits: 2 })}`;
}

const TABLE_ID_MATERIAL = 'prn-create-bom-material';
const TABLE_ID_ITEMS = 'prn-create-bom-items';
    ...
    <div className={cn('w-full flex-1 min-h-0 flex flex-col', className)}>
      <Tabs defaultValue='material' className='flex flex-col flex-1 min-h-0'>
        ...
        <TabsContent value='service-item' className='flex-1 min-h-0 mt-2'>
          <ServiceItemPrnTable projectId={projectId} />
        </TabsContent>
      </Tabs>
    </div>
  );
