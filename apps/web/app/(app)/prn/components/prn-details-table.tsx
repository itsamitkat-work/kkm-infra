...
import { usePrnDetailsQuery } from '../hooks/use-prn-details-query';
import {
  usePrnCheckMutation,
  usePrnVerifyMutation,
} from '../hooks/use-prn-check-verify-mutation';
import type { PrnDetailItem } from '../api/prn-api';
import { cn } from '@/lib/utils';
import { IconCheck } from '@tabler/icons-react';

interface PrnDetailsTableProps {
  prnCode: string;
  className?: string;
}
  ...
  return (
    <div className={cn('p-4 overflow-x-auto', className)}>
      <Table>
        ...
        <TableBody>
          {rows.map((row: PrnDetailItem, index: number) => (
            <TableRow key={row.prnDetailsHashId}>
