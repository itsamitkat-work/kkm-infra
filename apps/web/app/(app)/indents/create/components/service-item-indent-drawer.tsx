...
import {
  useCreateIndentServiceItemMutation,
...
export type ServiceItemIndentDrawerMode = 'create' | 'edit';

interface ServiceItemIndentDrawerProps {
  projectId: string;
  open: boolean;
  onOpenChange: (open: boolean) => void;
  mode: ServiceItemIndentDrawerMode;
  /** When mode is 'edit', the row to update */
  initialRow?: IndentServiceItemRow | null;
}

export function ServiceItemIndentDrawer({
  projectId,
  open,
  onOpenChange,
  mode,
  initialRow,
}: ServiceItemIndentDrawerProps) {
  ...
  const updateMutation = useUpdateIndentServiceItemMutation(projectId);
  ...
  const defaultServiceItemId =
    isEdit && initialRow ? initialRow.projectItemHashId : '';
  const defaultQuantity =
    isEdit && initialRow ? String(initialRow.serviceItemQuantity) : '1';
  ...
  React.useEffect(() => {
    ...
    if (isEdit && initialRow) {
      form.reset({
        serviceItemId: initialRow.projectItemHashId,
        quantity: String(initialRow.serviceItemQuantity),
      });
    } else {
    ...
    try {
      if (isEdit && initialRow) {
        await updateMutation.mutateAsync({
          projectItemId: initialRow.projectItemHashId,
          payload: {
            id: initialRow.indentServiceItemHashId,
            serviceItemCode: initialRow.serviceItemCode,
            serviceItemQty: Number(values.quantity),
            isChecked: 0,
            isVerified: 0,
          },
        });
  ...
  const title = isEdit
    ? 'Update Service Item Indent'
    : 'Add Service Item Indent';
  ...
  return (
    <DrawerWrapper open={open} onClose={() => onOpenChange(false)}>
      <FormDrawerHeader
        ...
        formId='service-item-indent-drawer-form'
        ...
        <AnimatePresence mode='wait'>
          {showLoading ? (
            ...
            >
              <Form {...form}>
                <form
                  id='service-item-indent-drawer-form'
